...
사용법:
    python 06_patch_obf.py [--no-restore] [--debug]
...
옵션:
    ...
    --debug         패치 상세 로그를 output/debug_obf_patch.json 에 저장.
...
def main():
    ...
    parser.add_argument('--debug', action='store_true',
                        help='패치 상세 로그를 output/debug_obf_patch.json 에 저장')
    ...
    debug_out = Path(resolve_path(paths['output'])) / 'debug_obf_patch.json' \
    ...
    stats = patch_jar(bak_jar, out_jar, translations, blocked_classes, jar_blocked,
                      label="obf", class_translations=class_trans, debug_out=debug_out)
